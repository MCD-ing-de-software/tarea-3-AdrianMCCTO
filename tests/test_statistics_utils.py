...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_basic_case(self):
        ...
        # Valores esperados: (1+2)/2=1.5, (2+3)/2=2.5, (3+4)/2=3.5
        expected = np.array([1.5, 2.5, 3.5])
        
        # Comparar arrays usando numpy.testing
        npt.assert_allclose(result, expected, rtol=1e-7, atol=1e-7)
    ...
    def test_moving_average_raises_for_invalid_window(self):
        ...
        # Verificar que window=0 lanza ValueError
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=0)
        
        # Verificar que window negativo lanza ValueError
        with self.assertRaises(ValueError):
            utils.moving_average(arr, window=-1)
        
        # Verificar que window mayor que la longitud del array lanza ValueError
        with self.assertRaises(ValueError):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        # Verificar que una secuencia 2D lanza ValueError
        with self.assertRaises(ValueError):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        # Verificar que la media es aproximadamente 0
        self.assertAlmostEqual(result.mean(), 0.0, places=7)
        
        # Verificar que la desviación estándar es aproximadamente 1
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        # Verificar que una secuencia con desviación estándar cero lanza ValueError
        with self.assertRaises(ValueError):
            utils.zscore(arr)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        # Verificar que el valor mínimo es 0.0
        ...
        # Para [2, 4, 6]: min=2, max=6, range=4
        # (2-2)/4=0.0, (4-2)/4=0.5, (6-2)/4=1.0
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        # Verificar que una secuencia con valores constantes lanza ValueError
        with self.assertRaises(ValueError):
